...
async function compileIndexHtml() {
  const infoRows = JSON.parse(
    await readFile(`${fieldbookDir}/home_page_info.json`)
  );
  const info = infoRows.reduce((info, row) => {
    info[row.name] = row.value;
    return info;
  }, {});
  const events = filterEvents(
    JSON.parse(await readFile(`${fieldbookDir}/events.json`))
  );
  const stats = JSON.parse(
    await readFile(`${fieldbookDir}/home_page_stats.json`)
  );
  const projects = JSON.parse(await readFile(`${fieldbookDir}/projects.json`));
  const testimonials = JSON.parse(
    await readFile(`${fieldbookDir}/testimonials.json`)
  );
